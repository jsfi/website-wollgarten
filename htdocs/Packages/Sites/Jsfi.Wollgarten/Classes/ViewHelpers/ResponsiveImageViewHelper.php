<?php
namespace Jsfi\Wollgarten\ViewHelpers;
...
class ResponsiveImageViewHelper extends AbstractTagBasedViewHelper
{
    ...
    public function render(ImageInterface $image = null, $width = null, $maximumWidth = null, $height = null, $maximumHeight = null, $allowCropping = false, $allowUpScaling = false, $async = false, $preset = null, $srcsetWidths = [])
    {
        ...
        if ($thumbnailData['width'] > 0 && $thumbnailData['height'] > 0) {
            ...
            if (!empty($srcsetWidths)) {
                $imageWidth = $thumbnailData['width'];
                $srcset = $thumbnailData['src'] . ' ' . $imageWidth . 'w';
                foreach ($srcsetWidths as $srcsetWidth) {
                    if ($srcsetWidth < $imageWidth) {
                        $thumbnailConfiguration = new ThumbnailConfiguration($srcsetWidth, null, null, $maximumHeight, $allowCropping, $allowUpScaling, $async);
                        $thumbnailData = $this->assetService->getThumbnailUriAndSizeForAsset($image, $thumbnailConfiguration, $this->controllerContext->getRequest());
                        $srcset .= ', ' . $thumbnailData['src'] . ' ' . $srcsetWidth . 'w';
                    }
                }
